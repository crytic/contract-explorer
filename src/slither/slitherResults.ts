...
export function getSanitizedDescription(result: SlitherResult): string {
  ...
  return result.description
    .replace(
      new RegExp(/\s{0,1}\(\S*\.sol(?:\#\d+\-\d+|\#\d+){0,1}\)/, "gi"),
      ""
    )
    .replace("\r\n", "\n");
}

export async function getResultElementRange(
  result: SlitherResult,
  elementIndex: number = 0,
  cleanerOverrides: boolean = true
  ...
  let endLine =
    resultElement.source_mapping.lines[
      resultElement.source_mapping.lines.length - 1
    ] - 1;
...
export async function gotoResultCode(
  ...
  try {
    ...
    if (result.elements.length <= 0 || !result.elements[0].source_mapping) {
      Logger.error(
        "Could not navigate to slither result. The result has no source mappings."
      );
      return;
    }
    ...
    if (!result._ext_in_sync) {
      Logger.error(
        "Could not navigate to slither result. The mapped source code has been modified."
      );
      return;
    }
    ...
    let filename_absolute = path.join(
      workspaceFolder,
      result.elements[0].source_mapping.filename_relative
    );
      ...
      vscode.window.showTextDocument(doc).then(async (editor) => {
        if (vscode.window.activeTextEditor) {
          ...
          let [startLine, startColumn, endLine, endColumn] =
            await getResultElementRange(result);
    ...
    // Log our error.
    Logger.error((r as Error).message);
  }
}
