...
function refreshCompilationTargetDropDown(
  selectedIndex = null,
  forceCompilationDataReload = false
) {
  // Cache our controls.
  ...
  if (selectedIndex < compilations.length && selectedIndex >= 0) {
    ...
    if (typeof compilation.targetType === "string") {
      switch (compilation.targetType) {
        case "basic": {
          $("#radio_compilation_type_basic")
            .prop("checked", true)
            .trigger("change");
          break;
        }
        case "standard_json": {
          $("#radio_compilation_type_solc_standard_json")
            .prop("checked", true)
            .trigger("change");
          break;
        }
...
function setUnsavedBasicCompilationTarget() {
  ...
  if (selectedIndex < compilations.length && selectedIndex >= 0) {
    compilations[selectedIndex].targetBasic.target = $(
      "#compilation_target"
    ).val();
  }
}
...
function refreshWorkspaceFoldersList() {
  ...
  let selectedCompilationIndex = $("#dropdown_compilation_group").prop(
    "selectedIndex"
  );
  if (
    selectedCompilationIndex < compilations.length &&
    selectedCompilationIndex >= 0
  ) {
  ...
  if (typeof targetWorkspaceName !== "string") {
    $dropdown_workspace_name.prop(
      "selectedIndex",
      workspaceFolders.length > 0 ? 0 : -1
    );
    setUnsavedCompilationWorkspaceName();
  }
}
...
function setUnsavedCompilationWorkspaceName() {
  ...
  if (
    ...
    // We have a valid index, fetch our cwd workspace name for this compilation.
    compilations[selectedCompilationIndex].cwdWorkspace =
      selectedWorkspaceFolderName;
  }
}
