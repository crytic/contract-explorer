...
(function() {
    ...
    window.addEventListener('message', event => {
        ...
        switch (data.method) {
            ...
            case 'setAutogeneratedSolcStandardJson': {
                // Set our autogenerated solc standard json.
                setAutogeneratedSolcStandardJson(data.manifests);
                break;
            }
        }
    });
...
function autogenerateSolcStandardJson() {
    // Post our save configuration message.
    vscode.postMessage({method: 'autogenerateSolcStandardJson'});
}

function setAutogeneratedSolcStandardJson(manifests) {
    ...
    if (selectedIndex < compilations.length && selectedIndex >= 0) {
        ...
        compilation.targetStandardJson = { target: manifests[0]};
    }
}
...
function setUIEventHandlers() {
    ...
    $('#btn_autogenerate_standard_json').on('click', (event) => {
        autogenerateSolcStandardJson();
    })
...
function addCompilationGroup() {
    ...
    refreshCompilationGroupDropDown(compilations.length - 1, true);
}
...
function removeCompilationGroup() {
    ...
    if (selectedIndex < compilations.length && selectedIndex >= 0) {
        ...
        refreshCompilationGroupDropDown(Math.max(0, selectedIndex - 1), true);
    }
}
