...
export class DetectorFilterTreeProvider
  implements vscode.TreeDataProvider<DetectorFilterNode>
{
  ...
  public async populateTree() {
    ...
    for (let detector of slither.detectors) {
      ...
      if (config.userConfiguration.hiddenDetectors) {
        checked =
          config.userConfiguration.hiddenDetectors.indexOf(detector.check) < 0;
      }
      // Create the node for this detector and add it to the list.
      let detectorFilterNode: DetectorFilterNode = new DetectorFilterNode(
        detector,
        checked
      );
  ...
  public getChildren(
    element?: DetectorFilterNode
  ): DetectorFilterNode[] | Thenable<DetectorFilterNode[]> {
