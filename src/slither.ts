...
export let detectorsByCheck: Map<string, SlitherDetector> = new Map<
  string,
  SlitherDetector
>();
export const results: Map<string, SlitherResult[]> = new Map<
  string,
  SlitherResult[]
>();
export const out_of_sync_results: Set<SlitherResult[]> = new Set<
  SlitherResult[]
>();
...
export async function analyze(print: boolean = true): Promise<boolean> {
  ...
  if (
    ...
    Logger.log(
      "There are no open workspace folders to run slither analysis on."
    );
    return false;
  }
  ...
  for (let i = 0; i < vscode.workspace.workspaceFolders.length; i++) {
    ...
    let { output, error } = await exec_slither(
      ". --json -",
      false,
      workspaceFolder
    );
    ...
    if (!error) {
      try {
        ...
        if (
          ...
          commandResult.results.detectors
        ) {
          ...
          error =
            "Could not obtain slither results: An unknown error occurred.";
        }
      } catch (e) {
        error = output;
      }
...
${error!.toString()}`
  ...
  for (let [workspaceFolder, workspaceResults] of results) {
    ...
    if (workspaceResults) {
      ...
      const resultsPath = config.getStorageFilePath(
        workspaceFolder,
        config.storageFiles.analysis
      );

      // Cache the results
      fs.writeFileSync(
        resultsPath,
        JSON.stringify(workspaceResults, null, "\t")
      );
  ...
  Logger.log("");
  Logger.log(
    `\u2E3B Analysis: ${successCount} succeeded, ${failCount} failed, ${
      vscode.workspace.workspaceFolders.length - (successCount + failCount)
    } skipped \u2E3B`
  );
...
export async function updateSourceMappingSyncStatus(
  firstTimeCalculation: boolean = false,
  fileNameFilter: string | undefined = undefined
) {
  ...
  for (let [workspaceFolder, workspaceResults] of results) {
    ...
    for (let workspaceResult of workspaceResults) {
      ...
      for (let workspaceResultElement of workspaceResult.elements) {
        ...
        let sourceMappingFile = path.join(
          workspaceFolder,
          workspaceResultElement.source_mapping.filename_relative
        );
        let sourceContent = sourceContentMap.get(
          workspaceResultElement.source_mapping.filename_relative
        );
        ...
        if (
          fileNameFilter &&
          path.normalize(sourceMappingFile) != path.normalize(fileNameFilter)
        ) {
          continue;
        }
        ...
        let mappedSource = sourceContent.substring(
          workspaceResultElement.source_mapping.start,
          workspaceResultElement.source_mapping.start +
            workspaceResultElement.source_mapping.length
        );
...
async function filterDuplicateResults(
...
export async function readResults(print: boolean = false): Promise<boolean> {
  ...
  for (let i = 0; i < vscode.workspace.workspaceFolders.length; i++) {
    ...
    if (fs.existsSync(resultsPath)) {
      ...
      let workspaceResults: SlitherResult[] = JSON.parse(
...
async function exec_slither(
  ...
  if (config.userConfiguration.solcPath) {
    ...
    Logger.log(
      `Invoking slither with custom solc path: "${config.userConfiguration.solcPath}"`
    );
  }
  ...
  let { stdout } = await cmd(`${config.slitherPath} ${args}`, {
